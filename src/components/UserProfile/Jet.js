...
const Jet = ({
  data = [100, 100, 85, 75, 75, 10, 30, 100, 100, 85, 75, 75],
  maxValue = 100,
  chartHeight = 80,
}) => {
  const [chartWidth, setChartWidth] = useState(
    window.innerWidth < 900 ? "100%" : 580
  ); // Default width
  ...
  const filledPoints = `0,0 ${getPoints(
    data,
    chartWidth,
    maxValue,
    chartHeight
  )} ${typeof chartWidth === "string" ? parseInt(chartWidth) : chartWidth},0`; // Starts from the top of the chart
